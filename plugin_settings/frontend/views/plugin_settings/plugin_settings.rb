class PluginSettings < Sequel::Model(:plugin_settings)
	...
	def self.init
		defs_file = File.join(ASUtils.find_base_directory("common"), "config", "plugin_settings_defaults.rb")
		defaults = {}
		if File.exists?(defs_file)
		  found_defs_file = true
		  Log.info("Loading plugin settings defaults file at #{defs_file}")
		...
		RequestContext.in_global_repo do
		  ...
		  if self.filter(filter).count == 0
			...
			if found_defs_file
			  ...
			  pref.update_from_json(JSONModel(:plugin_settings).from_hash({:defaults => defaults}),
  ...
  def self.current_settings(repo_id = RequestContext.get(:repo_id))
