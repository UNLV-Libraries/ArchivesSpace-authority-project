...
class ASFop
  ...
  def initialize(source, output= nil)  
   ...
   begin
	   @stylesheet = "as-ead-pdf-#{repo_id}.xsl"
	   @xslt = File.read( StaticAssetFinder.new(File.join('stylesheets')).find(@stylesheet)) 
   rescue 
	   #default to the normal stylesheet if StaticAssetFinder returns exception
	   @stylesheet = 'as-ead-pdf.xsl'
	   @xslt = File.read( StaticAssetFinder.new(File.join('stylesheets')).find(@stylesheet)) 
   end
  end
  ...
  def to_fo
    transformer = Saxon.XSLT(@xslt, system_id: File.join(ASUtils.find_base_directory, 'stylesheets', @stylesheet) )
