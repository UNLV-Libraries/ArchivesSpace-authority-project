...
AppConfig[:db_url] = "jdbc:mysql://dbmain:3306/SpecArcTest?user=specarct&password=AGcz4MtC&useUnicode=true&characterEncoding=UTF-8"
...
#AppConfig[:plugins] = ['local', 'aspace_feedback']
 AppConfig[:plugins] = ['local', 'unlv_importer', 'unlv_exporter', 'place', 'lcnaf']
#
...
AppConfig[:authentication_sources] = [{
                                         :model => 'LDAPAuth',
                                         :hostname => 'library.unlv.edu',
                                         :port => 389,
                                         :base_dn => 'ou=Users,ou=Support,dc=library,dc=unlv,dc=edu',
                                         :username_attribute => 'sAMAccountName',
                                         :attribute_map => {:cn => :name},
                                         :bind_dn => 'cn=System No Rights User,ou=Users,ou=Support,dc=library,dc=unlv,dc=edu',
                                         :bind_password => 'LTD-1243',
					}]
