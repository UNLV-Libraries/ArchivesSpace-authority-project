    ...
    while next_page <= last_page:
        ...
        for container in container_obj['results']:
            ...
            try:
                ...
                logging.error("Could not load container: %s" %(container), sys.exc_info()[0])
        next_page += 1

    # print "Dumping top containers: %s" % json.dumps(top_containers)

    # EACH spreadsheet row
    logging.info('Starting to run spreadsheet...')
        ...
        for row in reader:
            try:
                ...
                if not 'Location' in row.keys():
                    print "Location for this row not found: %s" % (row)
                    continue
                elif not 'Collection number' in row.keys():
                    print 'No collection number associated with %s it: %s' % (row['Location'].strip(), row)
                    continue
                elif not row['Collection number']:
                    print 'Collection number missing for %s: %s' % (row['Location'].strip(), row)
                    continue
                elif not row['Collection number'].startswith(('MS','PH')):
                    print 'Not a MS/PH in %s: %s' % (row['Location'].strip(), row)
                    continue
                ...
                for container in parse_containers(row['Container'], mss=row['Collection number']):
                    ...
                    if top_containers[container]['container_locations']:
                        print "Existing location for container %s: %s" % (container,json.dumps(top_containers[container]['container_locations']))
                        continue
                    ...
                    collection_title = '???'
                    if 'Collection Title' in row.keys() and row['Collection Title']:
                        collection_title = row['Collection Title']
                    print "Updated container %s (%s) location %s" % (container, collection_title, location_code)
            except:
                logging.error("Could not work with row: %s" % (row), sys.exc_info()[0])
    # Do any top containers still not have locations???
    logging.info('Checking for top containers still without a location')
    for container in top_containers.keys():
        if not top_containers[container]['container_locations']:
            print('Top container %s (%s) does not have a location' % (top_containers[container]['long_display_string'],top_containers[container]['uri']))
